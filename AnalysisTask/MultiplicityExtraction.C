...
void MultiplicityExtractiion(TString fileNameData1, TString fileNameData2, Bool_t corr1 = kFALSE, Bool_t corr2 = kFALSE)
{
  ...
  for (Int_t i = 0; i < 2; ++i) {
    ...
    fhNtrk[i] = static_cast<THnSparse*>(file->FindObjectAny(corr[i] ? "CMUL_hNtrkCorr" : "CMUL_hNtrk"));
    if (!fhNtrk[i]) return;
  }
