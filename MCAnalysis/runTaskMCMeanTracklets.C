...
void runTaskMCMeanTracklets(TString smode = "local", TString inputFileName = "runListLHC16p.txt",
                          Bool_t applyPS = kTRUE, Bool_t applyPileupCuts = kTRUE,
                          Bool_t isMC = kTRUE, TString refInput = "PYTHIAProfile.root")
{
  ...
  pathList.Add(new TObjString("/home/alidock/Quarkonia_Analysis/MCAnalysis"));
  TList fileList; fileList.SetOwner();
  fileList.Add(new TObjString("runTaskMCMeanTracklets.C"));
  ...
  // --- grid specific setup ---
  TString dataDir = "/alice/sim/2017/LHC17d18";
  TString dataPattern = "AOD209/*AliAOD.root";
  TString runFormat = "%d";
  TString outDir = "OutputQuarkonia/LHC17d18MCProfile";
...
//  if (applyPS) mult->SelectCollisionCandidates(AliVEvent::kMuonUnlikeLowPt7);
  */
  ...
  AliAnalysisTaskMeanTracklets* meanTracklets = reinterpret_cast<AliAnalysisTaskMeanTracklets*>(gROOT->ProcessLineSync("AddTaskMeanTracklets()"));
  if(!meanTracklets) {
